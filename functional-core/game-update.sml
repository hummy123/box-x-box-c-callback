...
struct
  ...
  fun handleBallInput (ball: ball, input): ball =
    let
      val {i, j, k, l} = input
      val {player, xPos, yPos, xMove, yMove} = ball
      ...
      val xPos = 
        if xPos >= 450 then 450
        else if xPos <= ~500 then ~500
        else xPos
    in
      {player = player, xPos = xPos, yPos = yPos, xMove = xMove, yMove = yMove}
    end

  fun update (game: game_board, input) : game_board =
    let
      val
        { dayBall
        , nightBall
      ...
      val nightBall = handleBallInput (nightBall, input)
